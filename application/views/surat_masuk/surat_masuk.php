<?php
    $status_session = $this->session->status_query_sm;
    ...
    $this->session->mark_as_temp('status_query_sm', 1);

    $status_session_update = $this->session->status_query_sm_update;
    ...
    $this->session->mark_as_temp('status_query_sm_update', 1);

    $status_session_hapus = $this->session->status_query_sm_hapus;
    ...
    $this->session->mark_as_temp('status_query_sm_hapus', 1);
...
<div class="row">
    <div class="col-lg-12">
        <h1 class="page-header"><i class="fas fa-file"></i> Surat Masuk</h1>
    </div>
</div>

<div class="row">
    <div class="col-lg-12">
        <a href="<?php echo base_url('index.php/surat_masuk/tambah'); ?>" class="btn btn-primary" ><i class="fas fa-file"></i> Tambah</a>
    </div>
</div>
...
<script type="text/javascript">
    $(document).ready(function() {
        $('#example').DataTable( {
            "processing": true,
            "serverSide": true,
            "ajax": "<?php echo base_url('index.php/surat_masuk/ambil_surat_masuk'); ?>",
            "columnDefs": [
                { "width": "15%", "targets": [0, 5] }
            ]
        });
    });
</script>
...
<div class="modal fade" id="hapus_surat_masuk" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-body">
...
<script>
    $('#hapus_surat_masuk').on('show.bs.modal', function(e) {
    ...
    $('#show_detail').on('show.bs.modal', function (e) {
        ...
        $.ajax({
            type : 'post',
            url : '<?php echo base_url('index.php/surat_masuk/detail'); ?>',
            data :  'rowid1='+ rowid1,
